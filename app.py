...
# # background-image: url("https://images.hdqwalls.com/wallpapers/blue-white-material-design-4k-up.jpg");
...
def display_forecast(forecast, range):
    ...
    second_month = forecast_months[1]
    third_month  = forecast_months[2]
    ...
    analysis.markdown(f"""
            # Forecast Analysis:\n
            - Lowest price = \${round(forecast.lstm_default.min(),2)} on {forecast.DATE[low_date]}
            - Highest price = \${round(forecast.lstm_default.max(),2)} on {forecast.DATE[high_date]}
    ...
    for new in news:
        with st.container():
            ...
            with thumbnail:
                ...
                except(FileNotFoundError): # Not all articles have thumbnails
